...
const WORDS = {
  fr: {
    ...
    errors: {
      ...
      scope: 'périmètre absent de la source', browser: 'erreur navigateur', unknown_kind: 'adaptateur inconnu', unavailable: 'source indisponible',
    },
  },
  en: {
    ...
    errors: {
      ...
      scope: 'scope missing from source', browser: 'browser error', unknown_kind: 'unknown adapter', unavailable: 'source unavailable',
    },
  },
};
...
export function errorText(failure, lang = 'fr') {
  ...
  if (failure.kind === 'unavailable') return detail ?? word;
