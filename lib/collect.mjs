...
import { INCIDENT_STATES, isActiveMaintenanceState, MAINTENANCE_STATES, safeExternalUrl, STATUS_LIMITS, validateStatusDocument } from '../public/status-contract.js';
...
export function buildOutput(providers, settled, now, adapters = {}) {
  ...
  const doc = {
    ...
    summary: {
      ...
      activeMaintenances: out.reduce((n, p) => n + p.maintenances.filter((m) => isActiveMaintenanceState(m.state)).length, 0),
    },
    providers: out,
  };
