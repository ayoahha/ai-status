...
export async function collectTencent(provider, get) {
  ...
  try {
    ...
    if (!res.ok) return { status: 'inconnu', collect: { state: 'error', error: `HTTP ${res.status} sur DescribeProductEventForRegionInPeriod` } };
    const categories = (await res.json())?.Response?.Data?.CategoryList;
    if (!Array.isArray(categories)) {
      return { status: 'inconnu', collect: { state: 'error', error: 'schéma DescribeProductEventForRegionInPeriod inattendu' } };
    }
    const products = categories.flatMap((c) => c.ProductList ?? []).filter((p) => ids.includes(p.ProductId));
    if (products.length === 0) {
      return { status: 'inconnu', collect: { state: 'error', error: `aucun produit du périmètre (${ids.join(', ')}) pour la région ${region}` } };
    }
