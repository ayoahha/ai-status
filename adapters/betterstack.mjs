...
export async function collectBetterstack(provider, get) {
  const url = provider.source.url.replace(/\/+$/, '') + '/index.json';
  try {
    ...
    if (typeof aggregate !== 'string' || !Array.isArray(included)) {
      return { status: 'inconnu', collect: { state: 'error', error: 'schéma index.json inattendu (aggregate_state ou included absents)' } };
    }
    const resources = included.filter((i) => i.type === 'status_page_resource');
    if (resources.length === 0) {
      return { status: 'inconnu', collect: { state: 'error', error: 'aucune ressource dans index.json' } };
    }
