...
| Perplexity | https://status.perplexity.com | Page passée à incident.io. Qualification du 12/09/2026 : Website, App et Computer visibles ; API non confirmée. Source déclarée non vérifiable, aucune requête de collecte. Ancien adaptateur conservé |
...
| OpenRouter | https://status.openrouter.ai | Page passée à Datadog ; données publiques dans `/config.json`. Qualification du 12/09/2026 : API Gateway et Web & Application Services visibles ; couverture models/generation et authentification non confirmée. Source déclarée non vérifiable, aucune requête de collecte. Ancien adaptateur conservé |
...
Les fixtures `statuspage-cloudflare-replicate.json`, `incidentio-perplexity-qualification.json` et `datadog-openrouter-qualification.json` contiennent des extraits des réponses publiques observées. Les deux dernières servent à constater les limites de couverture, pas à déclarer ces fournisseurs opérationnels. La qualification incident.io lit les données JSON embarquées par Next.js sans exécuter les scripts ; la page indique `expose_status_summary_api: false`. Datadog charge `/config.json`, qui contient sept services terminaux et `maintenances: null` ; le code public de la page traite cette valeur comme une liste vide. La couverture du périmètre déclaré reste néanmoins non confirmée.
